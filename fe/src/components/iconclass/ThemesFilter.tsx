...
const ThemeFilterItem: FC<ThemeFilterItemProps> = ({ iccParent }) => {
...
const ThemesFilter: FC<ThemesFilterProps> = ({
  isMobile,
  filterOpen,
  setFilterOpen,
}) => {
  ...
  return (
    ...
    >
      ...
      {!iccLoading && !iccError && icc?.items ? (
        ...
        >
          {filteredICC.map((i) => (
            <ThemeFilterItem key={i.icc.id} iccParent={i} />
          ))}
        </ul>
      ) : null}
      {isMobile && (
