...
import cz.inqool.eas.common.exception.v2.ConflictObject.Details.DetailsBuilder;
...
import static cz.inqool.eas.common.exception.v2.ConflictObject.Details.Fields.*;

@Beta
public class ConflictObject extends EasException {

    private static final HttpStatus DEFAULT_HTTP_STATUS = HttpStatus.CONFLICT;


    public ConflictObject(String code) {
        super(code, DEFAULT_HTTP_STATUS.value());
    }

    public ConflictObject(String code, String message) {
        super(code, DEFAULT_HTTP_STATUS.value(), message);
    }

    public ConflictObject(String code, Throwable cause) {
        super(code, DEFAULT_HTTP_STATUS.value(), cause);
    }

    public ConflictObject(String code, String message, Throwable cause) {
        super(code, DEFAULT_HTTP_STATUS.value(), message, cause);
    }


    public ConflictObject details(Consumer<DetailsBuilder> detailsCustomizer) {
    ...
    public ConflictObject debugInfo(Consumer<DetailsBuilder> detailsCustomizer) {
        DetailsBuilder detailsBuilder = Details.builder();
        detailsCustomizer.accept(detailsBuilder);
