...
import cz.inqool.eas.common.exception.v2.InvalidAttribute.Details.DetailsBuilder;
...
import static cz.inqool.eas.common.exception.v2.InvalidAttribute.Details.Fields.*;

@Beta
public class InvalidAttribute extends EasException {

    private static final HttpStatus DEFAULT_HTTP_STATUS = HttpStatus.BAD_REQUEST;


    public InvalidAttribute(String code) {
        super(code, DEFAULT_HTTP_STATUS.value());
    }

    public InvalidAttribute(String code, String message) {
        super(code, DEFAULT_HTTP_STATUS.value(), message);
    }

    public InvalidAttribute(String code, Throwable cause) {
        super(code, DEFAULT_HTTP_STATUS.value(), cause);
    }

    public InvalidAttribute(String code, String message, Throwable cause) {
        super(code, DEFAULT_HTTP_STATUS.value(), message, cause);
    }


    public InvalidAttribute details(Consumer<DetailsBuilder> detailsCustomizer) {
    ...
    public InvalidAttribute debugInfo(Consumer<DetailsBuilder> detailsCustomizer) {
        DetailsBuilder detailsBuilder = Details.builder();
        detailsCustomizer.accept(detailsBuilder);
